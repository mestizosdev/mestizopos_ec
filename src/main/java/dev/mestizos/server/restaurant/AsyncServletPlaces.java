 ...
 */
@AllArgsConstructor
public class AsyncServletPlaces extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (pathInfo != null) {
            floor = pathInfo.substring(1);
        }
        else {
            floor = request.getParameter("floor");
        }

        val places = FinderRestaurant.getPlaces(session, floor);

        ByteBuffer content = ByteBuffer.wrap(new Gson().toJson(places).getBytes(StandardCharsets.UTF_8));
